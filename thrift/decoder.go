...
func decodeRequest(ctx context.Context, data types.IoBuffer, oneway bool, message Message) (cmd interface{}, err error) {
	...
	request.Header.Set("X-Govern-Service", fmt.Sprintf("%s@thrift", message.Name))
	request.Header.Set("X-Govern-Method", message.Name)

	request.Header.Set("service", fmt.Sprintf("%s@thrift", message.Name))
...
func decodeResponse(ctx context.Context, data types.IoBuffer, message Message) (cmd interface{}, err error) {
	...
	response.Header.Set("X-Govern-Service", fmt.Sprintf("%s@thrift", message.Name))
	response.Header.Set("X-Govern-Method", message.Name)

	response.Header.Set("service", fmt.Sprintf("%s@thrift", message.Name))
